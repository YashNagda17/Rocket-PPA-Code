...
def save_checkpoint(path: str | Path, model: RocketPPAQwenModel, normalizer: Normalizer) -> None:
    path = Path(path)
...
def load_checkpoint(path: str | Path, map_location: str | torch.device = "cpu") -> tuple[RocketPPAQwenModel, Normalizer]:
    ...
    base = AutoModel.from_pretrained(config.base_model_name, trust_remote_code=True, device_map=None)
